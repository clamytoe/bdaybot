...
def test_add_reminder_ok(monkeypatch):
    ...
    status = TEST_BOT.add_reminder(USER, ABD.datetime, TZ, "general")
...
def test_add_reminder_fail(monkeypatch):
    ...
    with pytest.raises(TypeError):
        TEST_BOT.add_reminder(USER, "not a datetime", TZ, "general")
    with pytest.raises(TypeError):
        TEST_BOT.add_reminder(USER, ABD, None, "general")
    status = TEST_BOT.add_reminder(USER, ABD, TZ, "general")
    assert status is False


def test_lookup_birthday():
    _ = db.create_birthday(USER, ABDTZ.datetime, TZ)
    user_info = TEST_BOT.lookup_birthday(USER)
